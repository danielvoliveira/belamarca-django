...
def create_images_resized_signals(instance, model):
    '''
    ...
    image_to_resize_attributes_names = []
    image_size_attributes_names = []
    model_name = model.__name__

    for attribute_name in model.__dict__.keys():
        if 'image_resize' in str(attribute_name) and '_id' not in str(attribute_name):
            image_to_resize_attributes_names.append(attribute_name)

        if 'image_size' in str(attribute_name) and '_id' not in str(attribute_name):
            image_size_attributes_names.append(attribute_name)
        ...
        if instace_image != None:
            ...
            for size in images_and_sizes_list:
                if size[0] == field_name:
                    output_size_desktop = tuple([int(size[1][0]), int(size[1][1])])
                    output_size_tablet = tuple(
                        [int(int(size[1][0])/2), int(int(size[1][1])/2)])
                    output_size_mobile = tuple(
                        [int((int(size[1][0])/2)/2), int((int(size[1][1])/2)/2)])

            rename = '{}{}{}{}'.format(
                model_name,
                instance.id,
                field_name,
                instace_image.original_filename
            )
            rename = hash_image_name(rename)
            rename = '{}-{}'.format(unidecode(instace_image.original_filename.replace(" ", "-")), rename)
...
def delete_images_resized_signals(instance, model):
    '''
    ...
    # de atributo.
    image_to_delete_attributes_names = []

    for attribute_name in model.__dict__.keys():
        if 'image_resize' in str(attribute_name):
            if '_id' not in str(attribute_name):
                image_to_delete_attributes_names.append(attribute_name)
